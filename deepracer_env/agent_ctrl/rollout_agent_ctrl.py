...
from deepracer_env.agent_ctrl.agent_ctrl_interface import AgentCtrlInterface
from deepracer_env.agent_ctrl.utils import (set_reward_and_metrics,
                                     send_action, get_wheel_radius,
                                     get_normalized_progress, Logger,
                                     get_relative_pos)
from deepracer_env.track_geom.constants import (AgentPos, TrackNearDist, ObstacleDimensions, ParkLocation)
from deepracer_env.track_geom.track_data import FiniteDifference, TrackData
from deepracer_env.track_geom.utils import euler_to_quaternion, pose_distance, apply_orientation
from deepracer_env.metrics.constants import StepMetrics, EpisodeStatus
from deepracer_env.cameras.camera_manager import CameraManager
from deepracer_env.common import ObserverInterface
from deepracer_env.log_handler.deepracer_exceptions import RewardFunctionError, GenericRolloutException
from deepracer_env.reset.constants import AgentPhase, AgentCtrlStatus, AgentInfo, ZERO_SPEED_AGENT_PHASES
from deepracer_env.reset.utils import construct_reset_rules_manager
from deepracer_env.utils import get_racecar_idx
from deepracer_env.constants import DEFAULT_PARK_POSITION
from deepracer_env.gazebo_tracker.trackers.get_link_state_tracker import GetLinkStateTracker
...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface, AbstractTracker):
    ...
    def _get_reset_poses(self, dist):
        ...
        """
        # It is extremely important to get the interpolated pose of dist
        # using center line first. And then use the center line pose to
        # interpolate the inner and outer reset pose.
        # If dist is directly used with inner lane and outer lane pose
    ...
    def _get_car_reset_model_state(self, spawn_dist, car_pose, episode_status):
        ...
        Args:
            spawn_dist (float): Distance from the start position calculated wrt the progress
                                percentage value where the reset car is supposed to be spawned.
            car_pose (Pose): Car pose to be used to calculate the new car pose of the reset car.

        Returns:
            ModelState: reset state
        '''
        # Closest object is identified to maintain the rule of resetting a certain distance behind an object.
        ...
        new_pose = self._check_for_invalid_reset_pose(pose=new_pose, dist=spawn_dist)
    ...
    def _judge_action_at_run_phase(self, episode_status, pause):
        ...
        if pause:
            ...
            if episode_status == EpisodeStatus.CRASHED.value:
                ...
                # and static obstacle
                # Calculate spawn_dist from current_progress to get the most accurate normalized
                # distance of the car (model_point) to the center line of the track.
                spawn_dist = self._data_dict_['current_progress'] * \
                ...
                # car pose is used.
                # Calculate spawn_dist_dist based on prev_progress to decide the reset location.
                # This is done since the previous step will have the correct
